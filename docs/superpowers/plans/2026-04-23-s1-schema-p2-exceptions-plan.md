# S1 消息 Schema + P2 异常类体系 实现计划

> **给执行者：** 必须使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务执行本计划。步骤使用复选框（`- [ ]`）语法追踪进度。

**目标：** 实现结构化 SIP 消息 Schema（S1）和分层异常体系（P2），为所有后续模块建立应用层消息基础。

**架构：** 在 `sip_protocol/` 下新增两个子包：`schema/` 承载消息类型（枚举、数据类、验证），顶层 `exceptions.py` 承载异常层级。这些是纯数据结构，零加密/传输依赖——可以独立开发和测试。
...
├── exceptions.py              # 新增：全部异常类（包根目录，跨层使用）
├── schema/                     # 新增：SIP 消息 Schema
│   ├── __init__.py           # 公共导出
│   ├── types.py              # MessageType, Priority, RecipientType, ErrorSeverity
│   ├── parts.py              # TextPart, DataPart, FileRefPart, FileDataPart, ToolRequestPart, ToolResponsePart, ContextPart, StreamPart
│   ├── message.py            # SIPMessage dataclass, create/parse 工具函数
│   └── validation.py         # validate_message(), validate_parts()
python/tests/
├── test_exceptions.py         # 新增：异常类体系测试
└── test_schema.py            # 新增：消息 Schema 测试
```
...
git commit -m "docs: 更新CHANGELOG记录S1 Schema和P2异常类体系"
```
