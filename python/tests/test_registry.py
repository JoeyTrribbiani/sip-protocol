...
class TestQuery:
    ...
    def test_query_by_status_online(self):
        """按status=online过滤"""
        reg = AgentRegistry()
    ...
    def test_query_by_status_offline(self):
        """按status=offline过滤"""
        reg = AgentRegistry()
        reg.register(_make_card("OnlineBot"))
        reg.register(_make_card("OfflineBot"))
...
class TestHeartbeat:
    def test_heartbeat_success(self):
        """心跳成功时更新expires_at"""
        reg = AgentRegistry(RegistryConfig(default_ttl=300))
        card = _make_card("AgentA")
        reg.register(card)
    ...
    def test_heartbeat_offline_agent(self):
        """对离线Agent心跳返回False"""
        reg = AgentRegistry()
...
class TestHealthCheck:
    def test_check_health_marks_expired(self):
        """TTL过期后check_health将Agent标记为offline"""
        reg = AgentRegistry(RegistryConfig(default_ttl=10))
        reg.register(_make_card("AgentA"))
...
class TestCleanup:
    def test_cleanup_removes_expired_offline(self):
        """离线超过offline_ttl的Agent被清理"""
        reg = AgentRegistry(RegistryConfig(offline_ttl=60))
        reg.register(_make_card("AgentA"))
        reg._store["AgentA"].status = "offline"
    ...
    def test_cleanup_keeps_recent_offline(self):
        """离线未超过offline_ttl的Agent保留"""
        reg = AgentRegistry(RegistryConfig(offline_ttl=86400))
        reg.register(_make_card("AgentA"))
        reg._store["AgentA"].status = "offline"
...
class TestCustomConfig:
    def test_custom_config(self):
        ...
        config = RegistryConfig(default_ttl=5, offline_ttl=30)
