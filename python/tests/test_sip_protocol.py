...
def test_basic_handshake():
    ...
    print(
        f"✅ Agent A公钥：{pub_a.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw).hex()[:32]}..."
    )
    print(
        f"✅ Agent B公钥：{pub_b.public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw).hex()[:32]}..."
    )
